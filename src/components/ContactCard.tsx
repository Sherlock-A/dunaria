...
const LABELS: Record<
  Locale,
  { role: string; location: string; whatsapp: string; call: string; soon: string; response: string }
> = {
  es: {
  ...
  fr: {
    ...
    call: "Appeler",
    soon: "Prochainement",
    response: "Répond en moins d’une heure",
  },
};
...
export function ContactCard({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="w-full max-w-sm rounded-2xl border border-sand-300 bg-white shadow-sm overflow-hidden">
      {/* Identity */}
      <div className="flex flex-col items-center px-6 pt-8 pb-6 text-center">
        <div className="relative mb-4">
          <div className="h-24 w-24 rounded-full overflow-hidden ring-2 ring-gold ring-offset-2 ring-offset-white">
            <Image
          ...
          {/* Online dot */}
          <span className="absolute bottom-0.5 right-0.5 h-4 w-4 rounded-full bg-green-500 ring-2 ring-white" />
        </div>

        <p className="font-display text-xl font-medium text-night leading-tight">
          M. Youssef El Khannoubi
        </p>
        <p className="mt-1 font-mono text-xs uppercase tracking-widest text-gold">
          {l.role}
        </p>
        <p className="mt-2 flex items-center gap-1.5 text-xs text-night-600">
        ...
        <p className="mt-1.5 text-[11px] text-night-600/50 italic">{l.response}</p>
      </div>

      <div className="mx-6 border-t border-sand-300" />
      ...
      <div className="flex flex-col gap-3 px-6 py-5">
        <a
          ...
          className="flex items-center justify-center gap-2.5 rounded-xl py-3 text-sm font-medium text-white transition-opacity hover:opacity-90 active:scale-[.98]"
        ...
        <a
          href={telHref}
          className="flex items-center justify-center gap-2.5 rounded-xl border border-gold py-3 text-sm font-medium text-gold transition-colors hover:bg-gold/5 active:scale-[.98]"
        >
          ...
          {l.call}&nbsp;<span className="font-mono">{phone}</span>
        </a>
      </div>
      ...
      <div className="flex items-center justify-center gap-5 px-6 py-4">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-night-600/60 transition-colors hover:text-night"
          aria-label="Instagram @dunaria"
        >
          <InstagramIcon />
          <span className="font-mono text-xs">@dunaria</span>
          <span className="rounded-full bg-sand-200 px-1.5 py-0.5 font-mono text-[10px] text-gold">
            {l.soon}
          </span>
        </a>
